 ...
 */
public class hod_list extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_hod_list2, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        mSearchBar = getActivity().findViewById(R.id.chat_tab2_searchBar2);
        mSearchBtn = getActivity().findViewById(R.id.chat_tab2_searchBtn2);
    ...
    private void searchUserfromFb(String name) {
        ...
        FirebaseRecyclerOptions<UserModel> options =
                new FirebaseRecyclerOptions.Builder<UserModel>()
                        ...
                        .build();
        mAdapter = new FirebaseRecyclerAdapter<UserModel, hod_list.UserViewHolder>(options) {
            @Override
            protected void onBindViewHolder(@NonNull hod_list.UserViewHolder holder, int position, @NonNull final UserModel model) {
                holder.setDetails(model.getUsername(), model.getEmail(), model.getImage_url(),getContext());
                holder.mView.setOnLongClickListener(new View.OnLongClickListener() {
                    @Override
                    public boolean onLongClick(View v) {
                        final CharSequence[] items = {"Change role to Faculty", "Block User","Unblock User"};
                        ...
                        builder.setItems(items, new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                if (which == 0) {
                                    changeHodToFaculty(model.getUserID());
                                } else if (which == 1) {
            ...
            @NonNull
            @Override
            public hod_list.UserViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                View view = LayoutInflater.from(parent.getContext())
                        .inflate(R.layout.user_row, parent, false);

                return new hod_list.UserViewHolder(view);
            }
        };
    ...
    private void getHodList() {
        ...
        Query query = usersRef.orderByChild("role").equalTo("hod");
        FirebaseRecyclerOptions<UserModel> options =
                new FirebaseRecyclerOptions.Builder<UserModel>()
                        .setQuery(query, UserModel.class)//Query(streamsDbRef, liveStreamModel.class)
                        .build();
        mAdapter = new FirebaseRecyclerAdapter<UserModel, UserViewHolder>(options) {
            ...
            protected void onBindViewHolder(@NonNull UserViewHolder holder, int position, @NonNull final UserModel model) {
                ...
                holder.mView.setOnLongClickListener(new View.OnLongClickListener() {
                    @Override
                    public boolean onLongClick(View v) {
                        final CharSequence[] items = {"Change Hod to Faculty", "Block User","Unblock User"};
                        AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
                        builder.setTitle("Options");
                        builder.setItems(items, new DialogInterface.OnClickListener() {
                            @Override
    ...
    private void changeHodToFaculty(String userID){
        DatabaseReference facultyRef=FirebaseDatabase.getInstance().getReference("users").child(userID);
        facultyRef.child("role").setValue("faculty");
        Toast.makeText(getContext(), "HOD removed", Toast.LENGTH_SHORT).show();
    }
    private void initializations() {
        recyclerView = getActivity().findViewById(R.id.hod_list_recyclerView);
